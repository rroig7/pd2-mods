						...
						if addon_id and type(addon_data) == "table" then 
							if addon_id == true then --megapack
								...
								for crosshair_id,crosshair_data in pairs(addon_data) do 
									if type(crosshair_data) == "table" then
										...
										self:log("Error: LoadCrosshairAddons() " .. addon_lua_file_path .. " returned invalid data in sub-item. Expected results: [string],[table]. Got: [" .. type(crosshair_id) .. "] " .. tostring(crosshair_id) .. ", [" .. type(crosshair_data) .. "] " .. tostring(crosshair_data) .. ".")
									end
								end
								self:log("Done loading Crosshairs megapack " .. foldername .. ".")
							else -- individual addon, not megapack
							...
							self:log("Error: LoadCrosshairAddons() " .. addon_lua_file_path .. " returned invalid data. Expected results: [string],[table]. Got: [" .. type(addon_id) .. "] " .. tostring(addon_id) .. ", [" .. type(addon_data) .. "] " .. tostring(addon_data) .. ".")
						end
					end
				end
						...
						if addon_id and type(addon_data) == "table" then 
							if addon_id == true then --megapack
								...
								for hitmarker_id,hitmarker_data in pairs(addon_data) do 
									if type(hitmarker_data) == "table" then
										if type(hitmarker_data.parts) == "table" then 
											load_addon_textures(addon_path,foldername,hitmarker_data.parts)
										else
											self:log("Error: LoadHitmarkerAddons() megapack " .. addon_lua_file_path .. " returned invalid data. Expected results: [string],[table]. Got: [" .. type(hitmarker_id) .. "] " .. tostring(hitmarker_id) .. ", [" .. type(addon_data) .. "] " .. tostring(addon_data) .. ".")
											break
										end
										self:AddCustomHitmarker(hitmarker_id,hitmarker_data)
									else
										self:log("Error: LoadHitmarkerAddons() " .. addon_lua_file_path .. " returned invalid data in sub-item. Expected results: [string],[table]. Got: [" .. type(hitmarker_id) .. "] " .. tostring(hitmarker_id) .. ", [" .. type(hitmarker_data) .. "] " .. tostring(hitmarker_data) .. ".")
									end
								end
							...
							self:log("Error: LoadHitmarkerAddons() " .. addon_lua_file_path .. " returned invalid data. Expected results: [string],[table]. Got: [" .. type(addon_id) .. "] " .. tostring(addon_id) .. ", [" .. type(addon_data) .. "] " .. tostring(addon_data) .. ".")
						end
					end
				end
				if not is_advanced then 
			...
			for _,foldername in pairs(file_util:GetFolders(addon_path)) do 
				...
				if file_util:FileExists(Application:nice_path(addon_lua_file_path)) then
					...
					elseif addon_lua then 
						...
						if addon_id and type(addon_data) == "table" then
							if addon_id == true then --megapack
								self:log("Loading Hitsounds megapack " .. foldername .. "...")
								for hitsound_id,hitsound_data in pairs(addon_data) do 
									if type(hitsound_data) == "table" then
										if type(hitsound_data.variations_paths) == "table" then 
											hitsound_data.variations = hitsound_data.variations or {}
											for i,variation in pairs(hitsound_data.variations_paths) do
												--write full path to soundfile which includes addon path
												hitsound_data.variations[#hitsound_data.variations + 1] = path_util:Combine(full_addon_path,variation)
											end
										end
										if hitsound_data.path_local then 
											hitsound_data.path = path_util:Combine(full_addon_path,hitsound_data.path_local)
										end
										self:AddCustomHitsound(hitsound_id,hitsound_data)
									else
										self:log("Error: LoadHitsoundAddons() " .. addon_lua_file_path .. " returned invalid data in sub-item. Expected results: [string],[table]. Got: [" .. type(hitsound_id) .. "] " .. tostring(hitsound_id) .. ", [" .. type(hitsound_data) .. "] " .. tostring(hitsound_data) .. ".")
									end
								end
								self:log("Done loading Hitsounds megapack " .. foldername .. ".")
							else -- individual addon, not megapack
							...
							self:log("Error: LoadHitsoundAddons() " .. addon_lua_file_path .. " returned invalid data. Expected results: [string],[table]. Got: [" .. type(addon_id) .. "] " .. tostring(addon_id) .. ", [" .. type(addon_data) .. "] " .. tostring(addon_data) .. ".")
						end
					end
				end
				if not is_advanced then
